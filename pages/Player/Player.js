import React from 'react';
import Link from 'next/link'
import { Title } from '../Styles'
import Arbo from './../../components/Arbo'
import Header from './../../components/Header'


export default function Player() {

    const results =
    {
        name: "Ludovic Lefevre", performance: "1", regulariter: "10", cossy: "0303645252", resu: [
            { place: '15', event: 'GCG', date: '26/09/2021', deck: 'Data', rank: "B+" },
            { place: '4', event: 'TTT cup', date: '26/10/2020', deck: 'Data', rank: "B++" },
            { place: '1', event: 'World', date: '26/09/2048', deck: 'Insecte', rank: "S++" },
        ]
    };

    return (
        <div style={{
            minHeight: "100vh", overflowX: "hidden", backgroundColor: "#22171c", backgroundImage: "url(" + "/pattern.png" + ")", width: "100%",
            height: "100%",
            backgroundRepeat: "no-repeat",
            backgroundSize: "30%",
            backgroundPosition: "right top",
        }}>
            <style jsx global>{`
      body {
        margin: 0px;
        padding: 0px;
      }
    `}</style>
            <div style={{
                display: "flex",
                flexDirection: "row",
            }}>
                 <Arbo/>
                <div style={{ width: "100%", marginLeft: 30 }}>
                <Header/>
                    <div style={{ border: "3px solid", borderColor: '#0d8d40', borderRadius: "30px",display: "flex", flexDirection: "column", minHeight: "100vh", marginTop: 10, marginLeft: 10, marginRight: 30 }}>
                        <div style={{ display: "flex", flexDirection: "row", justifyContent: "space-between", marginRight: 30, marginLeft: 30, marginTop: 30 }}>
                            <img src={"https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRDCXkzWUlkGNPg3kHNl1hIeihamQQLMQfcegMprnpstdjN59n3bg3S5TZ2r1gcpdMhGJc&usqp=CAU"} style={{ width: 150, height: 200, marginTop: 10, border: "3px solid", borderColor: '#0d8d40', }} />
                            <div style={{ display: "flex", flexDirection: "column", justifyContent: 'space-around' }}>
                                <div>
                                    <div style={{ fontSize: 23, textAlign: "left", marginTop: 0, marginBottom: 0, color: "#efefef" }}>
                                        {results.name}
                                    </div>
                                    <div style={{ fontSize: 23, textAlign: "left", marginTop: 0, marginBottom: 0, color: "#efefef" }}>
                                        {results.cossy}
                                    </div>
                                </div>
                                <div>
                                    {results.performance != 1 ?
                                        <div style={{ fontSize: 23, textAlign: "left", marginTop: 0, color: "#efefef" }}>
                                            Performance saison actuel : {results.performance}eme
            </div> :
                                        <div style={{ fontSize: 23, textAlign: "left", marginTop: 0, color: "#efefef" }}>
                                            Performance saison actuel : {results.performance}er
           </div>
                                    }

                                    {results.regulariter != 1 ?
                                        <div style={{ fontSize: 23, textAlign: "left", marginTop: 0, color: "#efefef" }}>
                                            Regulariter saison actuel : {results.regulariter}eme
            </div> :
                                        <div style={{ fontSize: 23, textAlign: "left", marginTop: 0, color: "#efefef" }}>
                                            Regulariter saison actuel : {results.regulariter}er
           </div>
                                    }
                                </div>
                            </div>
                            <div style={{ display: "flex", flexDirection: "column", justifyContent: 'space-around' }}>
                                <div>
                                    {results.performance != 1 ?
                                        <div style={{ fontSize: 23, textAlign: "left", marginTop: 0, color: "#efefef" }}>
                                            Performance global : {results.performance}eme
            </div> :
                                        <div style={{ fontSize: 23, textAlign: "left", marginTop: 0, color: "#efefef" }}>
                                            Performance global : {results.performance}er
           </div>
                                    }
                                    {results.regulariter != 1 ?
                                        <div style={{ fontSize: 23, textAlign: "left", marginTop: 0, color: "#efefef" }}>
                                            Regulariter global : {results.regulariter}eme
            </div> :
                                        <div style={{ fontSize: 23, textAlign: "left", marginTop: 0, color: "#efefef" }}>
                                            Regulariter global : {results.regulariter}er
           </div>
                                    }
                                </div>
                                <div>
                                    {results.performance != 1 ?
                                        <div style={{ fontSize: 23, textAlign: "left", marginTop: 0, color: "#efefef" }}>
                                            Performance saison precedante : {results.performance}eme
            </div> :
                                        <div style={{ fontSize: 23, textAlign: "left", marginTop: 0, color: "#efefef" }}>
                                            Performance saison precedante : {results.performance}er
           </div>
                                    }
                                    {results.regulariter != 1 ?
                                        <div style={{ fontSize: 23, textAlign: "left", marginTop: 0, color: "#efefef" }}>
                                            Regulariter saison precedante : {results.regulariter}eme
            </div> :
                                        <div style={{ fontSize: 23, textAlign: "left", marginTop: 0, color: "#efefef" }}>
                                            Regulariter saison precedante : {results.regulariter}er
           </div>
                                    }
                                    <div style={{ fontSize: 23, textAlign: "left", marginTop: 0, color: "#efefef" }}>
                                        {results.rank}
                                    </div>
                                </div>
                            </div>
                            <img src={"data:image/jpeg;base64,"} style={{ width: 150, height: 200, marginTop: 10, border: "3px solid", borderColor: '#0d8d40', }} />

                        </div>


                        <div style={{ border: "2px solid", borderColor: '#0d8d40', marginLeft: "auto", marginRight: "auto", borderRadius: "30px", width: "90%", marginTop: 30}}>
                            {results.resu.map((result, index) => (
                                <div style={{
                                    display: "flex",
                                    flexDirection: "row",
                                    justifyContent: "space-around", 
                                }}>
                                    <div style={{ textAlign: "center", fontWeight: "bold", color: "#eaeaea", marginTop: 15, fontSize: 25, width: "15%" }}>
                                        {result.date}
                                    </div>
                                    <div style={{ textAlign: "center", fontWeight: "bold", color: "#eaeaea", marginTop: 15, fontSize: 25, width: "10%"  }}>
                                        {result.place}
                                    </div>
                                    <div style={{ textAlign: "center", fontWeight: "bold", color: "#eaeaea", marginTop: 15, fontSize: 25, width: "25%"  }}>
                                        <Link href={{
                                            pathname: "/Event/Events",
                                            query: { name: result.event },
                                        }}>
                                            <a>
                                                {result.event}
                                            </a>
                                        </Link>
                                    </div>
                                    <div style={{ textAlign: "center", fontWeight: "bold", color: "#eaeaea", marginTop: 15, fontSize: 25, width: "15%"  }}>
                                        {result.deck}
                                    </div>
                                    <div style={{ textAlign: "center", fontWeight: "bold", color: "#eaeaea", marginTop: 15, fontSize: 25, width: "10%" }}>
                                        {result.rank}
                                    </div>
                                </div>
                            ))}
                        </div>
                    </div>
                </div>
            </div>
        </div>
    )
}